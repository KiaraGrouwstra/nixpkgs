...
in
{
      ...
      type = nestedAttrsOf (
        submodule (
          { name, config, ... }:
          {
            options = {
              request = mkOption {
                description = "Request of the fileSecrets contract.";
                default = { };
                type = extend.request {
                  owner.default = "root";
                  group.default = "root";
              ...
              result = mkOption {
                description = "Result of the fileSecrets contract.";
                default = { };
                type = extend.result {
                  path.default = "/run/systemd-openbaod/files/${name}";
                };
              };
              ...
              # Vault-agent template for this secret, derived from openbaoPath/openbaoField.
              _template = mkOption {
                type = str;
                internal = true;
                default = ''{{- with secret "${config.openbaoPath}" }}{{ .Data.data.${config.openbaoField} }}{{- end }}'';
              };
            };
          }
        )
  ...
  config = lib.mkIf cfg.enable {
    ...
    contracts.${contract}.providers.systemd-openbaod = cfg.${contract};
