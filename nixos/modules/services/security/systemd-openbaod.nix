...
in
{
  # This module fetches its imported broker modules with `builtins.fetchTarball`
  # and reads `config`/`pkgs`, neither of which works in the sandboxed split
  # options doc build. Document it in the eager build instead.
  ...
  # The imported broker declares `services.openbao.agents` with freeform
  # `settings.*` sub-options that carry no `description`, which the NixOS manual
  # rejects (`warningsAreErrors`). These options are owned entirely by the
  # external broker source, not nixpkgs, so keep them out of the generated docs.
  options.services.openbao.agents = mkOption { visible = false; };
