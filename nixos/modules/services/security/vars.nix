...
let
  ...
  inherit (config.contracts.${generateFilesContract}) mkProviderType;
...
in
{
  # The contract provider options read `config.contracts` for their
  # `type`/`default`, which cannot be evaluated in the sandboxed split options
  # doc build. Document them in the eager build instead.
  ...
  options.services.vars = mkOption {
    ...
    type = submodule {
      ...
      options.${fileSecretsContract} = mkOption {
        ...
        type = config.contracts.${fileSecretsContract}.mkProviderType { };
      };
    };
  };
