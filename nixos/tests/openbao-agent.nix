{ pkgs, lib, ... }:
{
  ...
  nodes.machine =
    { config, pkgs, ... }:
    {
      ...
      # `services.openbao` has no dev mode, and a sealed server would need an
      # init and unseal dance this test has no opinion about.
      systemd.services.openbao-dev = {
        description = "OpenBao dev server";
        wantedBy = [ "multi-user.target" ];
        after = [ "network.target" ];
        # Dev mode writes the root token through the token helper, whose `~`
        # it expands by shelling out. NixOS emits no fallback `PATH`, and a
        # system unit has no `HOME` for `~` to resolve to.
        path = [ pkgs.bash ];
      ...
      environment = {
        ...
        variables = {
          # Literal v4 rather than `localhost`: `-dev-listen-address` binds
          # 127.0.0.1 only, and the client tries `::1` first.
