...
# results are scoped automatically so each service reads results via just its option name.
#
...
{ lib, pkgs, ... }:
{
  ...
  nodes.machine =
    ...
    let
      ...
      inherit (config.contractDefinitions.arithmetic) mkContract;

      # Inner sub-service: lives inside outer, requests value = 10.
      # Reads results via scoped direct injection - just `results.operation`.
      innerModule =
        { lib, config, ... }:
        {
          _class = "service";
