...
let
  ...
  inherit (dbContract) mkContract extend;
in
{
  ...
  nodes.machine =
    { config, pkgs, lib, ... }:
    let
      # Shared base submodule for a single database contract instance.
      dbInstanceModule = {
        options = {
      ...
      fakedbModule =
        { lib, config, contracts ? { }, name, ... }:
        {
              ...
              types.submodule [
                ...
                (
                  { config, name, ... }:
                  {
                    ...
                    config.result.socketPath = lib.mkDefault "/run/fakedb/${config.request.name}";
                  }
                )
              ]
            );
          ...
          config = {
            fakedb = contracts.database.requests or { };

            contract.providers.database.${name} = config.fakedb;
            ...
            process.argv = [
              ...
              (lib.concatStringsSep "\n" (
                lib.flatten (
                  lib.mapAttrsToList (_consumer:
                    lib.mapAttrsToList (_instance:
                      lib.mapAttrsToList (_db: v:
                        "mkdir -p ${lib.escapeShellArg v.result.socketPath}"
                      )
                    )
                  ) config.fakedb
                )
    ...
    in
    {
      ...
      assertions = [
        {
          assertion =
            config.contracts.database.results.webapp.instance.db.socketPath
            == "/run/fakedb/webapp_instance";
