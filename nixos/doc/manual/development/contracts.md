...
Contract type definitions live in `lib/contracts/definitions/`; the registry is `lib.contracts`. NixOS auto-seeds `config.contractDefinitions` from there, and other module systems can do the same by importing `lib.contract.module` and setting `config.contractDefinitions = lib.contracts`.
...
A contract type lives under `lib/contracts/definitions/` and exposes `meta` plus an `interface` of `request` and `result` options:

```nix
# lib/contracts/definitions/my-contract.nix
{ lib, ... }:
let
...
Register it in `lib/contracts/definitions/default.nix`:
...
User-defined contract types — added directly to `config.contractDefinitions.<name>` rather than to nixpkgs — are also picked up automatically.
