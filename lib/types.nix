            ...
            mergeAttr =
              ...
              if misshapen != null then
                throw ''
                  ...
                  expected an attribute set with keys from [${lib.concatStringsSep ", " (lib.filter (k: k != "_module") (lib.attrNames elemOptAttrs))}],
                ...
                ''
              else if goDeeper then self.merge (loc ++ [ name ]) subDefs
              else elemType.merge (loc ++ [ name ]) subDefs;
          in
